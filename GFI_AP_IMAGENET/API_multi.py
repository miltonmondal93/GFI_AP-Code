...
import torchvision, sys, math, copy, os, time, tqdm, sklearn.metrics, platform, getpass, hashlib, multiprocessing
...
class Models:
    ...
    def __init__(self, model, num_layers, num_transition_shape=None, num_linear_units=None, num_class=None,
        ...
        if model == 'ResNet':
            if (num_transition_shape != None) or (num_linear_units != None):
                print('Warning: ResNet ignores num_transition_shape, num_linear_units.')
        if self._pretrained:
            print('Loading pretrained model.')

    def net(self):
        if self._model == 'VGG':
            net = self._VGG_(self._num_layers, self._num_transition_shape, self._num_linear_units, self._num_class,
                             self._pretrained)
            return net
        if self._model == 'ResNet':
            net = self._ResNet_(self._num_layers, self._num_class, self._pretrained)
            return net
    ...
    class _Agnostic_():
        ...
        def start_training(self, dataset, eval_freq, epoch, preprocessFn=None, loss_in_db=False, final_eval=True):
            ...
            for e in range(1, epoch + 1):
                ...
                for data, labels in dataset.train_images:
                    ...
                    if self._global_step % eval_freq == 0:
                        ...
                        if loss_in_db: loss_value = -10 * np.log10(loss_value.cpu())
                        ...
                        print('Iter: %d, Loss %.4f Acc(Train Eval Top5) %.2f %.2f %.2f @%dim/s' % (
                            self._global_step, loss_value,
                            training_accuracy,
                            top1, top5, speed
                        ))
                        collate_loss, correct_predictions, total_processed = torch.zeros(1).cuda(), torch.zeros(
        ...
        def distill_training(self, dataset, eval_freq, epoch, teacher, preprocessFn=None, loss_in_db=False, final_eval=True):
            collate_loss, correct_predictions, total_processed = torch.zeros(1).cuda(), torch.zeros(
                1).cuda(), torch.zeros(1).cuda()
            tic = time.time()
            #return
    ...
    class _ResNet_(nn.Module, _Agnostic_):
        ...
        def __init__(self, num_layers, num_class, pretrained=False):
            super().__init__()

            assert num_layers in [18, 34, 50, 101, 152, 20, 32, 44, 56,
                                  110], 'num_layers must be [18, 34, 50, 101, 152, 20, 32, 44, 56, 110]'
        ...
        def dry_run(self, dataset, num_iterations=10):
            if self._optim is None: raise ConnectionError(
                'Optimizer not connected. Use attach_optimizer to connect optimizer.')
            iteration = 0
            for data, labels in dataset.train_images:
        ...
        def get_features(self, dataset, num_batches, after_layer, layer_type='conv', return_type='tensor',
                         verbose=False):

            assert layer_type in ['conv', 'bn', 'relu'], 'layer_type must be [\'conv\', \'bn\', \'relu\']'
            assert return_type in ['tensor', 'mean'], 'return_type must be either [' + ' '.join(
                ['tensor', 'mean']) + ']'
            ...
            if return_type == 'tensor':
                ...
                # print('2', self._num_class)
                features_sum = [features_sum[i].__mul__(torch.Tensor([1 / labels_set[i]])).cpu().numpy() if labels_set[
                                                                                                                i] != 0 else np.float32(
                    [0]) for i in range(self._num_class)]
        ...
        def prune(self, layer, filter, verbose=True):
            ...
            assert pruning_config in ['CBC', 'DBSBCS', 'DBSBL', 'SBDBCS',
                                      'SBDBL'], 'Error: No suitable pruning config found.'

            if pruning_config == 'CBC':
                if verbose: print('Pruning config:' + pruning_config,
    ...
    class _VGG_(nn.Module, _Agnostic_):
        ...
        def get_features(self, dataset, num_batches, after_layer, layer_type='conv', return_type='tensor',
            ...
            assert layer_type in ['conv', 'bn', 'relu'], 'layer_type must be either [' + ' '.join(
                ['conv', 'bn', 'relu']) + ']'
            assert return_type in ['tensor', 'mean'], 'return_type must be either [' + ' '.join(
                ['tensor', 'mean']) + ']'
            ...
            for i, (data, labels) in enumerate(dataset.train_images):
                ...
                with torch.no_grad():
                    features = self.forward(data.cuda(), layer=after_layer, layer_type=layer_type)
        ...
        def prune(self, layer, filter, verbose=True):
            ...
            if pruning_config == 'CBC':
                ...
                prune_conv_layer(third_idx, filter, 'in')
                if verbose: print(
                    'Pruning conv(out):fil=' + str(filter) + ', bn:elem=' + str(filter) + ', conv(in):idx=' + str(
                        filter))
                ...
                if verbose: print(
                    'Pruning conv(out):fil=' + str(filter) + ', bn:elem=' + str(filter) + ', linear(col):idx=' + str(
                        filter))
            elif pruning_config == 'LL':
                prune_linear_layer(first_idx, filter, 'row')
                prune_linear_layer(second_idx, filter, 'col')
                if verbose: print(
                    'Pruning linear(row):idx=' + str(filter) + ', bn:elem=' + str(filter) + ', linear(col):idx=' + str(
                        filter))
...
class Datasets:
    ...
    def __init__(self, dataset, train_batch_size, eval_batch_size=512):
        ...
        if dataset == 'MNIST':
            ...
            self._train_dataset = torchvision.datasets.CIFAR10(self._root, train=True, download=True,
                                                               transform=torchvision.transforms.Compose([
            ...
            self._eval_dataset = torchvision.datasets.CIFAR10(self._root, train=False, download=True,
                                                              transform=torchvision.transforms.Compose([
                                                                  torchvision.transforms.ToTensor(),
                                                                  torchvision.transforms.Normalize(
                                                                      (0.485, 0.456, 0.406), (0.229, 0.224, 0.225)),
            ...
            self._train_dataset = torchvision.datasets.ImageNet(self._root, split='train',
                                                                transform=torchvision.transforms.Compose([
            ...
            self._eval_dataset = torchvision.datasets.ImageNet(self._root, split='val',
                                                               transform=torchvision.transforms.Compose([
        ...
        self.train_images = torch.utils.data.DataLoader(self._train_dataset, batch_size=train_batch_size, shuffle=True,
                                                        num_workers=40, pin_memory=True)
        self.eval_images = torch.utils.data.DataLoader(self._eval_dataset, batch_size=eval_batch_size, shuffle=False,
                                                       num_workers=40, pin_memory=True)
